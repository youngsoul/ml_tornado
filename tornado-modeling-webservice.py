...
def display_image_bits(normalized_image_data):
    line = ''
    for i,element in enumerate(normalized_image_data):
        if i != 0 and i %28 == 0:
            print(line)
            line = ''
        line += element
...
def create_normalized_bitmap(number_to_predict):
    font = ImageFont.truetype("overpass-regular.otf", 28)
    image = Image.new('1', (28, 28)) #create new image, 10x10 pixels, 1 bit per pixel
    draw = ImageDraw.Draw(image)
    draw.text((5, -5),number_to_predict,fill=255,font=font)
    #draw.ellipse((2, 2, 8, 8), outline ='white')
    #image_data = list(image.getdata())

    normalized_image_data = [f"{int(x/255)}" for x in image.getdata()]

    return normalized_image_data

normalized_image_data = create_normalized_bitmap(number_to_predict)
...
print(r.content)
display_image_bits(normalized_image_data)
